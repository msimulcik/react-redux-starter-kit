...
.reduce((collection, descriptors) => {
  descriptors.forEach(({ id, defaultMessage, description }) => {
    ...
    collection[id] = { defaultMessage, description }; // eslint-disable-line no-param-reassign
  });
  return collection;
}, {});
...
defaultMessages = messageKeys.reduce((acc, key) => {
  acc[key] = defaultMessages[key]; // eslint-disable-line no-param-reassign
  return acc;
}, {});

// Build the XLIFF document for the available languages
i18n.en = messageKeys.reduce((acc, key) => {
  acc[key] = defaultMessages[key].defaultMessage; // eslint-disable-line no-param-reassign
  return acc;
}, {});
