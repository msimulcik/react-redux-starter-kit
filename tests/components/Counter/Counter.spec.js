...
describe('(Component) Counter', () => {
  ...
  it('Should render with an <h2> that includes Sample Counter text.', () => {
    expect(_wrapper.find('h2').find(FormattedMessage)).to.have.prop('id', 'counter.label');
  });
  ...
  describe('An increment button...', () => {
    ...
    beforeEach(() => {
      _button = _wrapper.find('button').filterWhere(
        a => a.find(FormattedMessage).prop('id') === 'counter.increment'
      );
    });
  ...
  describe('A Double (Async) button...', () => {
    ...
    beforeEach(() => {
      _button = _wrapper.find('button').filterWhere(
        a => a.find(FormattedMessage).prop('id') === 'counter.double'
      );
    });
